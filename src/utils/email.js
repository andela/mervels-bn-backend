...
class Emails {
  ...
  static updateTemplate(data) {
    const from = process.env.FROM_EMAIL;
    const to = data.manager;
    const subject = ' BareFoot Update Notification ';
    const html = `
    <body style="margin: 0; padding: 0;">
    <table border="0" cellpadding="0" cellspacing="0" width="900px"style="padding: 0 40px 0 40px; background-color:#f1f2f3;">
        <tr>
            <td align="center" style="background-color:#f9fcff; margin: 0 50px 0 50px;">
              <a>
                <img src="http://drive.google.com/uc?export=view&id=1Rraew69PkllzCnb5wIBJsw61xPk83sA-"
 ...
 " alt="Bare Foot Nomad"
   ...
   * @param {Object} title email subject details
   * @returns {String} reset password email template
   */
  static rejectAcceptRequestTemplate(reason, manager, requester, title) {
    const from = manager.userEmail;
    const to = requester.userEmail;
    const subject = title;
    const html = `
      <body style="margin: 0; padding: 0;">
        <table border="0" cellpadding="0" cellspacing="0" width="900px" style="padding: 0 40px 0 40px; background-color:#f1f2f3;">
